...
- `/` - домашня сторінка
- `/userlist` - сторінка списку користувачів
- `/userlist/:userId` - сторінка деталей користувача
- `/userlist/:userId/posts` - вкладений маршрут інформації про пости
- `/userlist/:userId/todos` - вкладений маршрут інформації про туду
- `*` - компонент сторінки помилки

### Components

- `Navigation`
  - Компонент навігації до сторінок `/` та `/userlist`
...
- `UserDetailsPage`
  ...
  - Рендерить вкладену навігацію до маршрутів `/posts` та `/todos`
